...
def draw_boxes(image, boxes, classes, thickness=4):
    ...
    if (classes[0] == 2):
        ...
        text = 'NO LIGHT'
    draw.text((0, 0), text,(190,255,190),font=font)
    
#-------------------------------------------------------------------------
def draw_boxes_extra(image, boxes, classes, detection_time, scores, filepath, threshold, thickness=4):
    """Draw bounding boxes on the image"""
    draw = ImageDraw.Draw(image)
    for i in range(len(boxes)):
        bot, left, top, right = boxes[i, ...]
    ...
    if(scores[0] > threshold):
        if (classes[0] == 2):
            text = 'RED LIGHT'
            text2 = "Detection Time(sec):{0:f} | Score:{1:f}".format(detection_time,  scores[0])
        elif (classes[0] == 3):
            text = 'YELLOW LIGHT'
            text2 = "Detection Time(sec):{0:f} | Score:{1:f}".format(detection_time,  scores[0])
        elif (classes[0] == 1):
    ...
    draw.text((0, 0), text,(190,255,190),font=font)
    draw.text((0, 40), text2,(190,255,190),font=font)
    draw.text((0, 80), str(scores),(190,255,190),font=font)    
    draw.text((0, 120), filepath,(190,255,190),font=font)
    
#-------------------------------------------------------------------------
def Label_text(image, classes):
    import cv2
    ...
    cv2.putText(image, text, (500,150), font, fontScale=1.2, color=(190,255,190), thickness=5, lineType=cv2.LINE_AA)
...
#-----------------------------------------------------------------------------------
    def get_classification(self, image, index):
        ...
        if(self.is_debug_mode == True):
            ...
            draw_boxes_extra(image_sample, box_coords, classes, detection_time, scores, image, self.detection_confidence)
            mpimg.imsave("./processed_image_rosbag1/processed_"+str(index)+".png", image_sample, format='png')
...
if __name__ == '__main__':
    ...
    #light_classifier.get_classification("left0000.jpg", 999)
    
    import os
    files = os.listdir("rosbag01/")
    filePaths = []
    
    for file in files:
        filePath = "rosbag01/" + "".join(file)
    ...
    for filePath in filePaths:
        index = index + 1
        print(filePath)
        light_classifier.get_classification(filePath, index)

    """
    light_classifier.get_classification("green1.jpg", 0)
    light_classifier.get_classification("green2.jpg", 1)
    light_classifier.get_classification("red1.jpg", 2)
    light_classifier.get_classification("red2.jpg", 3)
    light_classifier.get_classification("yellow1.jpg", 4)
    light_classifier.get_classification("yellow2.jpg", 5)
    """